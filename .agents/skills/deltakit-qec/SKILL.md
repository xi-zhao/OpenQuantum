...
调用 Harness 注册的 `deltakit_local.run_deltakit_memory`。适用于比较 3×3、3×5、5×3、5×5 码片在 X/Z 存储实验中的表现，或查看 Deltakit 生成的实际含噪电路。

- `width` 和 `height` 是数据量子位码片尺寸，不是全部物理量子位数；实际资源以结果 `numQubits/numDetectors/numObservables` 为准。
- 首版只用 Deltakit `ToyNoise(p)`；它是合成噪声模型，不能将 p 解释为实测硬件保真度。最多 10 轮、8192 shots，保留 seed。
