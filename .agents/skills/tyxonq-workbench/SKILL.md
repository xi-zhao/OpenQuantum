...
description: 使用 OpenQuantum 的 TyxonQ 本地 MCP 构建并运行有界量子电路，比较无噪声 statevector 与 density-matrix 噪声采样结果。用于 TyxonQ 电路验证、Bell/GHZ 等小规模态制备、采样分布和退极化/振幅阻尼/相位阻尼/Pauli 噪声分析；不用于真实量子硬件、云任务、任意 Python 执行或替代独立科学 Validator。
---
...
这个 Skill 负责组织 TyxonQ 工作流，真正的计算由独立注册的 `tyxonq_local` MCP 完成。
...
2. 若工具不存在，告诉用户在“设置 → 量子组件 → MCP 组件”启用 **TyxonQ Local**，重启
   OpenQuantum 后再试；不要改用 Bash 绕过设置。
3. 把用户意图整理成明确的 `numQubits` 与 `operations`。只使用 MCP schema 支持的门，旋转角单位为弧度。
...
- 真实后端发现使用 `fieldqkit-hardware`；任何付费或真实硬件任务仍需独立 MCP 与 Harness 审批。
- TyxonQ 脉冲/TQASM 和量子化学能力尚未进入本地稳定 Interface，不要声称当前工具已经支持。

## 解释规则

- MCP 成功只代表 TyxonQ 本地计算完成，不代表算法、物理模型或科研结论通过科学验收。
...
- MCP 不可用或返回错误时保留原始错误语义，不编造状态、概率或版本。
