...
诊断对象是一份 `platform-diagnostics` 报告。报告中的每个检查项必须有可复核证据，最终状态由
Validator 根据检查结果推导，不能由模型自行宣称。
...
1. UI：确认 UI 只通过 Harness transport adapter 发出会话命令，不直接调用模型或 MCP。
2. Harness 配置：运行 `npm run harness:config`，确认组合能展开且默认模型属于 OpenQuantum route。
3. Harness Host：检查 Host 根页面、`session.list` 和 `llm.models` API；未启动时记录 `not_checked`，
   不把静态配置当成运行证据。
...
输出先给结论，再给证据和下一步。不要把运行成功等同于科学验收；具体量子能力应由各自 Skill
包内的 Validator 和 eval 再判定。
