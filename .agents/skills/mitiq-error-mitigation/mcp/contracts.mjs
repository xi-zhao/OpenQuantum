...
const gate = obj({
  ...
  targets: arr(int(0, 3), 1, 2),
  angle: num(-2 * Math.PI, 2 * Math.PI),
}, ["name", "targets"]);
const estimate = num(-1e8, 1e8);
const statistics = obj({ mean: estimate, bias: estimate, variance: num(0, 1e16), rmse: num(0, 1e8), meanStandardError: num(0, 1e8) });
const trace = obj({
  stage: { type: "string", enum: ["baseline", "evaluation", "calibration", "training"] },
  shots: int(1, 32768), estimate, gateCount: int(0, 120), depth: int(0, 120),
...
export const definition = defineScienceTool({
  ...
  description: "Compare Mitiq ZNE, REM, PEC or CDR with an equal-shot unmitigated baseline on a bounded local noisy circuit. Returns every replicate, empirical bias/variance/RMSE and calibration/training costs. No cloud or hardware execution; no scientific acceptance claim.",
  source: { name: "mitiq", version: "1.1.0", repository: "https://github.com/unitaryfoundation/mitiq", license: "GPL-3.0-only" },
  inputSchema: obj({
    method, numQubits: int(1, 4, 2),
    gates: { ...arr(gate, 1, 24), default: [
      { name: "H", targets: [0] }, { name: "RZ", targets: [0], angle: 0.7 },
      { name: "H", targets: [0] }, { name: "CX", targets: [0, 1] },
    ] },
    observable: { type: "string", pattern: "^[IXYZ]{1,4}$", default: "ZI" },
    depolarizingProbability: num(0, 0.1, 0.02),
    readoutProbability: num(0, 0.2, 0),
    shotsBudget: { ...int(1024, 32768, 8192), description: "Shots per comparison arm per replicate, including calibration/training. Entire call uses exactly 2 * replicates * shotsBudget simulated shots." },
    replicates: { ...int(4, 16, 8), description: "Independent repetitions of the complete procedure, including new training/calibration data." }, seed: int(0, 2147483647, 7),
    pecSamples: int(16, 128), trainingCircuits: int(4, 24),
  ...
  checkInput(v) {
    ...
    if (v.shotsBudget * v.replicates > 262144) throw new Error("shotsBudget times replicates must be <=262144 per comparison arm");
  ...
  resultSchema: obj({
    ...
    noiseConvention: { type: "string", minLength: 1, maxLength: 800 },
    statistics: obj({ unmitigated: statistics, mitigated: statistics, rmseDifference: estimate }),
    totalShotsIncludingBaseline: int(8192, 524288),
    trials: arr(obj({
      seed: int(0, 4294967295), unmitigated: estimate, mitigated: estimate,
      baselineShots: int(1024, 32768), mitigationShots: int(1024, 32768),
      calibrationShots: int(0, 32768), trainingShots: int(0, 32768), evaluationShots: int(1, 32768),
      classicalReferenceEvaluations: int(0, 24),
      circuits: arr(trace, 2, 130),
      details: obj({
        scaleFactors: arr(num(1, 5), 3, 3), pecOneNorm: num(1, 1e8),
        sampledSigns: arr({ type: "integer", enum: [-1, 1] }, 16, 128),
        confusionMatrices: arr(arr(arr(num(0, 1), 2, 2), 2, 2), 1, 4),
        targetCounts: arr(int(0, 32768), 2, 16),
        trainingIdeal: arr(num(-1.00000001, 1.00000001), 4, 24),
        trainingNoisy: arr(num(-1, 1), 4, 24),
        regressionConditionNumber: num(1, 1e8),
      }, []),
    }), 4, 16),
  }),
});
