...
支持结构化分子、已安装 PySCF 目录中的命名基组和可选 activeSpace；默认 H₂/STO-3G。坐标用 Å，能量用 Hartree。提供 molecule 后以其 atoms/charge 构建分子，默认键长不参与几何。使用闭壳层 RHF 轨道，活性电子数为偶数，n_alpha=n_beta 固定 M_s=0，不约束总自旋；活性轨道从冻结核之后连续选取。轨道数、样本数和子空间尺寸由用户选择。
