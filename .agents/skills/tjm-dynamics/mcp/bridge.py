...
def compute(v):
    ...
    if reference["status"] == "computed":
        ...
        def derivative(_time, flat):
            ...
            return dr.ravel()
        solution = solve_ivp(derivative, (0.,float(times[-1])), rho.ravel(), t_eval=times, rtol=1e-9, atol=1e-11)
        if not solution.success:
            raise ValueError("Dense Lindblad reference did not converge")
        matrices = solution.y.T.reshape(-1,dim,dim)
    ...
    return {"times": times.tolist(), "siteZ": values.tolist(), "referenceSiteZ": ref_z.tolist() if ref_z is not None else None,
        ...
        "referenceTraceError": float(max(abs(np.trace(r)-1) for r in matrices)) if matrices is not None else None}, [
        ...
        "A completed tensor calculation does not establish convergence in time step, bond dimension, or trajectory count; a skipped dense reference supplies no accuracy estimate."]
