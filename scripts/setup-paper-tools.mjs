...
const supported = [...ids, "hamiltonian-simulation", "qcut-knitting", "compact-optimization", "openqarp-excited-states", "cqlib-kernel", "flagquantum-workbench", "mitiq-error-mitigation", "dynamiqs-dynamics", "clifft-sampling", "oqupy-dynamics", "deltakit-qec", "pyzx-optimization", "graphix-mbqc", "symmer-tapering", "paulie-algebra"];
...
for (const id of selected.length ? selected : ids) {
  ...
  const run = spawnSync(julia ? "julia" : "uv", julia
    ...
    env: { ...environment, UV_PROJECT_ENVIRONMENT: path.join(root, ".openquantum/python-envs", id), CARGO_HOME: path.join(root, ".openquantum/cargo-cache"), JULIA_NUM_PRECOMPILE_TASKS: "2", JULIA_PKG_PRECOMPILE_AUTO: "0" },
  });
