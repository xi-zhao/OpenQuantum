...
export function retrieveQuantumPractice(value) {
  ...
  const output = [
    ...
    ...(workflow ? [`Adapted native Skill: ${workflow.skill}. ${workflow.algorithm ? `Open SDK example: examples/quantum-algorithms/run.py --algorithm ${workflow.algorithm}; execute through the existing Harness bash/pwsh Tool after explicit capability:algorithms:setup. Scope: ${workflow.scope}` : workflow.invocation === "manual" ? "This category Skill is retained for explicit user invocation. For automatic tasks use quantum-algorithms to select a method Skill directly." : "Load this local Skill for backend guidance, workflow selection, or open-source migration."}`] : []),
