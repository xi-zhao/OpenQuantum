...
test("FlagQuantum launcher sanitizes credentials and freezes the isolated environment", async t => {
  ...
  const child = spawn(process.execPath,[entrypoint],{env:{...process.env,PATH:`${sandbox}${path.delimiter}${process.env.PATH}`,OPENAI_API_KEY:"secret-sentinel",IBM_QUANTUM_TOKEN:"secret-sentinel"},stdio:"ignore"});
