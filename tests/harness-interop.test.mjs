...
const CASES = [...INTEROP_TOOLS,
  { ...INTEROP_TOOLS[1], id: "qbraid-invalid", input: { gates: [{ gate: "CX", targets: [0, 0] }] }, expectError: true },
];
...
test("Harness discovers interop Skills, runs three real integrations and persists results and rejection", { skip: !enabled, timeout: 180000 }, async (t) => {
  const root = process.cwd();
  const sandbox = await mkdtemp(path.join(tmpdir(), "oq-interop-harness-"));
  ...
  const model = createServer(async (request, response) => {
    ...
    dispatched = true;
    const delta = initial ? { role: "assistant", tool_calls: CASES.map((c, index) => ({ index, id: `interop-fixture-${index}`, type: "function", function: { name: toolNames[index], arguments: JSON.stringify(c.input) } })) } : { role: "assistant", content: "Three local interoperability actions completed and one invalid circuit was rejected as expected; scientificValidation=not_evaluated." };
    ...
    ]) response.write(`data: ${JSON.stringify({ id: "interop-fixture", object: "chat.completion.chunk", created: 1, model: body.model, ...item })}\n\n`);
  ...
  const sessionId = `session-interop-${crypto.randomUUID()}`;
  await rpc("session/create", { sessionId, cwd: root, agentPreset: "openquantum" });
  const skillList = await rpc("skills/list", { sessionId });
  for (const c of INTEROP_TOOLS) assert.ok(skillList.skills.some(skill => skill.name === c.id && skill.modelInvocable), c.id);
  await rpc("session/prompt", { sessionId, mode: "queue", content: [{ type: "text", text: "Run the four fixed local interoperability cases and return their actual evidence." }] });
  ...
  }, "completed interoperability Harness turn", 120000);
  ...
  for (const [index, capability] of CASES.entries()) {
    ...
    const result = events.find(event => event.type === "tool/result" && JSON.stringify(event).includes(`interop-fixture-${index}`));
    ...
    if (capability.server) {
      const definition = (await import(`../.agents/skills/${capability.id}/mcp/${capability.contract}`))[capability.exportName];
  ...
  const replay = await harnessSessionSnapshot(base, cookie, sessionId, 500);
  assert.deepEqual(replay.records.filter(entry => entry.event?.type === "tool/result"), history.records.filter(entry => entry.event?.type === "tool/result"));
