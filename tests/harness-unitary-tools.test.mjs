...
const CASES = [...UNITARY_TOOLS,
  { id: "metriq-data", tool: "metriq_benchmarks", input: { action: "search", provider: "origin", limit: 1 } },
  { id: "metriq-data-invalid", tool: "metriq_benchmarks", input: { action: "get", id: "0".repeat(64) }, expectError: true },
];
...
test("Harness discovers four ecosystem Skills, runs five real Tools and persists success and failure results", { skip: !enabled, timeout: 600000 }, async (t) => {
  const root = process.cwd();
  const sandbox = await mkdtemp(path.join(tmpdir(), "oq-unitary-harness-"));
  ...
  const model = createServer(async (request, response) => {
    ...
    dispatched = true;
    const delta = initial ? { role: "assistant", tool_calls: CASES.map((c, index) => ({ index, id: `unitary-fixture-${index}`, type: "function", function: { name: toolNames[index], arguments: JSON.stringify(c.input) } })) } : { role: "assistant", content: "Five local ecosystem actions completed and one missing-record lookup failed as expected; scientificValidation=not_evaluated." };
    ...
    ]) response.write(`data: ${JSON.stringify({ id: "unitary-tools-fixture", object: "chat.completion.chunk", created: 1, model: body.model, ...item })}\n\n`);
  ...
  const sessionId = `session-unitary-${crypto.randomUUID()}`;
  await rpc("session/create", { sessionId, cwd: root, agentPreset: "openquantum" });
  const skillList = await rpc("skills/list", { sessionId });
  for (const c of UNITARY_TOOLS) assert.ok(skillList.skills.some(skill => skill.name === c.id && skill.modelInvocable), c.id);
  await rpc("session/prompt", { sessionId, mode: "queue", content: [{ type: "text", text: "Run the six fixed small-system unitary-tool cases and return their actual evidence." }] });
  ...
  }, "completed unitary-tools Harness turn", 540000);
  ...
  for (const [index, capability] of CASES.entries()) {
    ...
    const result = events.find(event => event.type === "tool/result" && JSON.stringify(event).includes(`unitary-fixture-${index}`));
    ...
    if (capability.expectError) {
      ...
      assert.match(serialized, /Unknown Metriq record/);
      continue;
    }
    ...
    if (capability.server) {
      ...
      assert.equal(output.source.commit, "6730f78b135a9af67691a0ef4fbea041978056c5");
      assert.ok(output.records.length > 0);
      assert.equal(output.records[0].provider, "origin");
    }
  }
  assert.equal(sawModelToolResult, true);
  const evidence = path.join(root, ".openquantum/unitary-tools-evidence");
