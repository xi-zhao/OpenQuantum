...
test("CC Connect upgrades only its exact legacy launcher and preserves private channel configuration", async (t) => {
  const root = await mkdtemp(path.join(tmpdir(), "openquantum-cc-upgrade-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  await materializeRuntime(root);
  const paths = resolveCcConnectPaths(root);
  const current = renderCcConnectConfig(root, "fixture-management-token-000000")
    .concat('\n[[projects.platforms]]\ntype = "feishu"\n[projects.platforms.options]\napp_secret = "fixture-private-value"\n');
  const legacy = current.replace(`cmd = ${JSON.stringify(paths.acpBin)}`,
    `cmd = ${JSON.stringify(path.join(root, "node_modules/.bin/dsh-acp-demo"))}`)
    .replace(`args = ["--profile", "acp", "--patch", ${JSON.stringify(paths.acpConfigPath)}]`,
      `args = ["--config", ${JSON.stringify(paths.acpConfigPath)}]`);
  await mkdir(paths.stateRoot, { recursive: true });
  await writeFile(paths.configPath, legacy, { mode: 0o600 });
  const options = { probe: async () => false };
  assert.equal((await ensureCcConnectConfig(root, options)).migrated, true);
  assert.equal(await readFile(paths.configPath, "utf8"), current);
  const backup = `${paths.configPath}.pre-dsh-0.1.5`;
  assert.equal(await readFile(backup, "utf8"), legacy);
  assert.equal((await stat(backup)).mode & 0o777, 0o600);
  assert.equal((await ensureCcConnectConfig(root, options)).migrated, false);
  for (const customized of [legacy.replace('cmd = ', 'cmd = "custom-wrapper" # '), legacy.replace('# Managed entrypoint generated by OpenQuantum', '# User configuration')]) {
    await writeFile(paths.configPath, customized);
    assert.equal((await ensureCcConnectConfig(root, options)).migrated, false);
    assert.equal(await readFile(paths.configPath, "utf8"), customized);
  }
});
...
test("CC Connect config is local, deterministic in shape, and never overwritten", async () => {
  ...
  assert.match(config, /--profile", "acp", "--patch/);
...
test("CC Connect ACP shares model composition and deployment defaults", () => {
  ...
  const acpById = new Map([...acpRows, ...insertedRows(acpRows)].map((row) => [row.id, row]));
  ...
  const acpAgent = acpById.get("acp");
  ...
  assert.match(acpById.get("system-prompt").config.personaPrefix, /trusted Acceptance Report or Result Commit/);

  assert.deepEqual(acpById.get("settings")?.config?.dshHome, acpById.get("credentials")?.config?.dshHome);
  assert.equal(acpById.get("llm-deepseek")?.disabled, true);
...
test("OpenQuantum ACP uses the shared settings.yaml model route override", { timeout: 60_000 }, async () => {
  ...
  await prepareOpenQuantumHarnessHome({ projectRoot, harnessHome: dshHome });
  await writeFile(
  ...
  const paths = resolveCcConnectPaths(projectRoot);
  const child = spawn(paths.acpBin, ["--profile", "acp", "--patch", paths.acpConfigPath], {
    cwd: projectRoot,
    env: {
      ...process.env,
      DSH_HOME: dshHome,
  ...
  try {
    ...
    const toolNames = modelRequest.body.tools.map((tool) => tool.function.name);
    assert.ok(toolNames.includes("solve_and_validate_ground_state"), `Missing native quantum tool: ${toolNames}`);
    assert.ok(toolNames.includes("mcp__qcec_local__verify_circuit_equivalence"), "MCP Tools must be mounted before the first prompt");
    assert.equal(child.exitCode, null, `ACP exited after its model turn\n${stderr}`);
    assert.doesNotMatch(stderr, /entries did not activate/);
  } catch (error) {
    assert.fail(`${error instanceof Error ? error.stack : error}\nACP error data: ${JSON.stringify(error?.data)}\nACP stderr:\n${stderr}`);
  } finally {
    child.stdin.end();
