...
首次接入 2026-09-12；计算接口更新 2026-09-15。六项以 L1 能力接入同一个 OpenQuantum Harness：每项包含实验选择 Skill 和一个计算 Tool。
研究价值来自方法与可复核实现，发表期刊本身不保证某个软件版本或算例正确。

| 能力 / 论文 | 固定实现 | 本次开放的 Tool 与科学范围 |
| --- | --- | --- |
| [SQD，Science Advances 2025](https://doi.org/10.1126/sciadv.adu9991) | [qiskit-addon-sqd 0.13.1](https://github.com/Qiskit/qiskit-addon-sqd)，Apache-2.0 | `sqd_local.run_sqd_chemistry`：分子与活性空间的采样子空间对角化、配置恢复；接受动态位宽 counts，可选同一活性空间 FCI 参照。 |
| [TJM，Nature Communications 2025](https://doi.org/10.1038/s41467-025-66846-x) | [MQT YAQS 0.6.0](https://github.com/munich-quantum-toolkit/yaqs)，MIT | `tjm_local.simulate_tjm_dynamics`：用户指定大小的开放 Ising 链、局域振幅衰减与张量跳跃轨迹；可选密度矩阵 Lindblad 参照。 |
| [LSD，Nature Communications 2025](https://doi.org/10.1038/s41467-025-63214-7) | [ldpc 2.4.1](https://github.com/quantumgizmos/ldpc)，MIT | `ldpc_local.decode_ldpc_syndromes`：二元校验矩阵和 syndrome 批次 的串行 BP+LSD 解码和独立 GF(2) 残差检查。 |
| [RandomMeas，Quantum 2026](https://doi.org/10.22331/q-2026-04-28-2086) | [RandomMeas.jl 0.3.1](https://github.com/bvermersch/RandomMeas.jl/tree/89c492bfb05508e5babe9c8c2c40697995be9e42)，Apache-2.0 | `random_meas_local.estimate_randomized_purity`：product/GHZ 态、局域 Haar 测量、指定子区纯度及有限样本误差。 |
| [Flow-VQE，npj Quantum Information 2025](https://doi.org/10.1038/s41534-025-01159-x) | [Flow-VQE f7642afa](https://github.com/olsson-group/Flow-VQE/tree/f7642afa330e5108ea5738d42fe80b551363733c)，MIT | `flow_vqe_local.train_flow_vqe`：实际调用论文的单上下文 flow 训练函数，适配 Pauli Hamiltonian 与无矩阵 RY/CNOT ansatz，比较等评估预算随机搜索。 |
| [TeNPy，SciPost Physics Codebases 2024](https://doi.org/10.21468/SciPostPhysCodeb.41) | [physics-tenpy 1.1.1](https://github.com/tenpy/tenpy)，Apache-2.0 | `tenpy_local.solve_tenpy_chain`：自旋 1/2 XYZ 开放链的 two-site DMRG、磁化、纠缠熵与收敛信息；可选精确对角化参照。 |
...
计算规模和预算由用户选择；参数、参考模式与实测记录见[计算规模与参考检查](SCALABLE_BRIDGES.md)。

## 计算约定与限制

- **SQD**：坐标与键长用 Å，能量用 Hartree。默认 H₂/STO-3G；可传分子、已安装 PySCF 命名基组及活性空间。
  闭壳层 RHF 轨道，固定 n_alpha=n_beta（M_s=0），不约束总自旋；位序为 beta(n-1)…beta0 alpha(n-1)…alpha0，保留前导零。
  HF 构型总是包含。`coreEnergyOffsetHartree` 已含核排斥和冻结核能量，返回总能量不能再加核排斥项。
  FCI 按 referenceMode 选择；required 可尝试任意请求规模；参照对象是同一活性空间 Hamiltonian。
  缺少 counts 时显式使用合成均匀样本，不能作为量子优势证据。有限样本可能只覆盖 HF 子空间而高于 FCI。
- **TJM**：`H=-JΣZZ-gΣX`，`hbar=1`，初态全零，局域跳跃 `sqrt(gamma)|0><1|`。
  时间与速率采用同一单位；严格包含 0 与请求终点，返回 `steps+1` 点。
...
- **RandomMeas**：下标从 0 开始，设置数和 shots 由用户指定。返回上游的有限 shots 偏差修正纯度估计，
...
- **Flow-VQE**：Pauli 最左字符对应 q0；实振幅 RY/CNOT ansatz 可能无法表达一般 Hamiltonian 的基态。
  评估预算由用户指定，精确参照按 referenceMode 选择；随机基线为 `[-π,π]` 均匀参数。上游训练用 float32，重算能量用 float64；
  这是单 Hamiltonian 上的训练，不包含论文分子数据、预训练权重、跨分子泛化或优化加速结论。临时训练文件自动回收。
- **TeNPy**：`S=Pauli/2`，`H=Σ(Jx SxSx+Jy SySy+Jz SzSz)-Σ(hx Sx+hz Sz)`。
  当前 two-site sweep 需要至少三站点；站点数、bond dimension 与 sweeps 由用户指定。
  小系统能量差、sweep 收敛条件和截断误差分别报告；有限 sweep/bond 的执行结束不保证全局基态。

SQD、TJM、Flow-VQE 和 TeNPy 均接受 `referenceMode=auto|required|skip`。主计算与独立参考分开；未运行参考时显式返回 `not_run` 和 null 参考字段，保持 L1。
