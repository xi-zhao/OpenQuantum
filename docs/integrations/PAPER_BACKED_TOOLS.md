...
首次接入 2026-09-12；执行配置更新 2026-09-15。六项以 L1 能力接入同一个 OpenQuantum Harness：每项包含实验选择 Skill 和一个有界计算 Tool。
...
| [TJM，Nature Communications 2025](https://doi.org/10.1038/s41467-025-66846-x) | [MQT YAQS 0.6.0](https://github.com/munich-quantum-toolkit/yaqs)，MIT | `tjm_local.simulate_tjm_dynamics`：开放 Ising 链、局域振幅衰减与张量跳跃轨迹；可选密度矩阵 Lindblad 参照。 |
...
SQD、TJM、Flow-VQE 和 TeNPy 的系统规模与计算投入由用户选择，执行参数及独立参考见[计算工具使用说明](SCALABLE_BRIDGES.md)。本地验证算例单独记录。

## 计算约定与限制

- **SQD**：坐标与键长用 Å，能量用 Hartree。默认 H₂/STO-3G；可传 H–Ne 分子、三种基组及用户指定的活性空间。
  ...
  FCI 参照对象是同一活性空间 Hamiltonian，可由 `referenceMode` 选择。PySCF CI 编码要求少于 64 个活性空间轨道。
  缺少 counts 时显式使用合成均匀样本，不能作为量子优势证据。有限样本可能只覆盖 HF 子空间而高于 FCI。
- **TJM**：`H=-JΣZZ-gΣX`，`hbar=1`，初态全零，局域跳跃 `sqrt(gamma)|0><1|`。
  时间与速率采用同一单位；严格包含 0 与请求终点，返回 `steps+1` 点；步数与轨迹数由用户指定。单条含噪轨迹的标准误为 null。
...
- **Flow-VQE**：Pauli 最左字符对应 q0；实振幅 RY/CNOT ansatz 可能无法表达一般 Hamiltonian 的基态。
  系统规模、层数、epochs 和 batch size 由用户指定；随机基线为 `[-π,π]` 均匀参数。上游训练用 float32，重算能量用 float64；
  这是单 Hamiltonian 上的训练，不包含论文分子数据、预训练权重、跨分子泛化或优化加速结论。临时训练文件自动回收。
- **TeNPy**：`S=Pauli/2`，`H=Σ(Jx SxSx+Jy SySy+Jz SzSz)-Σ(hx Sx+hz Sz)`。
  当前 two-site sweep 需要至少三站点；链长、bond dimension 与 sweeps 由用户指定。
  小系统能量差、sweep 收敛条件和截断误差分别报告；有限 sweep/bond 的执行结束不保证全局基态。

SQD、TJM、Flow-VQE 和 TeNPy 均接受 `referenceMode=auto|required|skip`。`auto` 按默认算例规模选择参考，`required` 显式运行参考，`skip` 省略参考；未运行参考时显式返回 `not_run` 和 null 参考字段，保持 L1。
...
每个连接一次运行一个计算，输入/输出由 schema 检查，整个子进程组支持取消和超时。默认工作进程 180 秒、输出 2 MiB；SQD、TJM、Flow-VQE 与 TeNPy 的 `execution` 可调整时间、线程和输出预算。
