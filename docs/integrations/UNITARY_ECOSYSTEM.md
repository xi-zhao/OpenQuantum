...
| [Clifft](https://github.com/unitaryfoundation/clifft) | PyPI 0.10.0；Apache-2.0 | 1–128 qubit Clifford+T、门后独立去极化、最终 Z 测量，按 active width 与采样预算控制成本 | `clifft_local.sample_clifft_circuit` |
...
**Clifft** 支持 H、S、T、X、Y、Z、CX、CZ，1–128 qubits、最多 2048 门、8192 shots，并检查输出与实际 active-width 预算。门后对该门涉及的各量子位施加 `DEPOLARIZE1(p)`，p 是三类 Pauli 错误的总概率。结果从左到右为 q0、q1……；独立密度矩阵参照至 6 qubits。`referenceMode=auto|required|skip` 控制参考；运行参考时返回完整位串，跳过时仅返回实际观察到的位串（observed_only），参考概率与 TVD 为 null。loss、leakage、续算与动态反馈仍未开放。新参数、组合预算与本地实测范围见[计算规模与参考检查](SCALABLE_BRIDGES.md)。
