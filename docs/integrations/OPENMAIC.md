...
整套应用已经装配并能启动；这不代表所有在线功能均已验收。上次真实模型验收遇到端点连接失败，本轮未重新验证该外部端点。新版 Harness + 本机模型桩 + 真实 SDK 的建课与持久化已通过；真实在线 AI 建课、问答和编辑仍需另行验收。
...
嵌入时 Pro 与普通首页之间使用原版的直接路由切换，避免共享元素快照动画；独立打开时保留原版动画。
...
- 源码提交：`29735f10d0081859ac3db1a50a0cc92f46436004`（v1.0.1 之后的四项修复）。
- 源码及依赖：`.openquantum/external/openmaic`，不进入 OpenQuantum Git。
- `learning:ui:setup` 校验提交、按已提交的集成锁文件安装依赖（Next.js `16.3.4`、React `19.3.0`、KaTeX `0.18.7`），显式构建工作区包和同步导入器。
...
上游 TypeScript、样式解析及 Harness Client 构建检查通过。

2026-09-10 浏览器复验：主系统和量子学习通首页已实际显示，品牌、字体、中性背景、按钮及浅色主题同步可见；
嵌入页 `data-oq-host-theme` 为 `light`。点击 Pro 触发了内嵌浏览器崩溃，因此已在嵌入边界采用原版无快照动画的路由分支。
上游 8 项模式切换测试与新增 4 项嵌入回归测试通过，类型及适配重放检查通过；该兼容修复仍待重新加载后的浏览器复验。
深色主题、Pro 工作台、课堂及编辑器的完整界面验收尚未完成。
...
| 项目 | 实际结果 |
| --- | --- |
| 原版首页及模型选择 | 浏览器已显示原版首页、材料附件、深度互动、PPTX 导入和 Pro 开关；2026-09-10 已查看品牌样式并确认浅色主题同步，Pro 切换崩溃的兼容修复待浏览器复验 |
