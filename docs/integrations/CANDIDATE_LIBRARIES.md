...
经现有 Harness MCP Client 注册为 Tool；没有另建运行时。原工作树已暂存的 Graphix/PyZX/Symmer/PauLie
适配保持独立，不计为本轮新增成果。[计划与去重决策](CANDIDATE_INTEGRATION_PLAN.md)。
...
并发互斥、超时、输出大小和失败协议。`execution` 可由调用者调整；没有人为 qubit/数据规模上限。
大任务仍受实际内存、上游算法和 Harness 工具超时限制，需同步调整连接超时。
...
Graphix 等既有暂存工作不重复接入。Dense-Evolution 的 BSL、polypus 的 EUPL/独立数据权利、stresscf 的缺失许可，
