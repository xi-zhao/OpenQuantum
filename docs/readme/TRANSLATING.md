...
The detailed editions follow capabilities → value → startup → results → development → roadmap → contribution → open-source acknowledgments. Compact editions may combine sections while retaining the same core claims and limitations. Keep first-run instructions together and optional detail in one level of expansion. Catalogs stay complete and grouped by purpose; source, activation policy and permissions remain separate fields. Add new entries to an existing group whenever appropriate.

Maintain one primary location for each installation step, capability limit, service setting and upstream attribution. Preserve established anchors when moving or renaming sections, and check incoming links from all language editions and the documentation index. Catalog checks verify that regrouping retains every entry and its declared behavior.
