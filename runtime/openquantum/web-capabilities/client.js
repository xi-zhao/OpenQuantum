globalThis.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = `
      ...
      @media(max-width:700px){.oq-cap-grid,.oq-cap-metrics,.oq-cap-form{grid-template-columns:1fr}.oq-cap-credential{grid-template-columns:1fr}.oq-cap-span{grid-column:auto}}
    `;
    ...
    const copy = {
      zh: {
        nav: "量子能力",
        title: "量子能力中心",
        summary: "Skill 描述领域工作流，MCP 提供确定性工具和量子后端。它们仍由 DeepSeek Harness 发现、运行与隔离；OpenQuantum 只提供量子配置视图。",
      },
      en: {
        nav: "Quantum",
        title: "Quantum capabilities",
        summary: "Skills describe domain workflows while MCP servers provide deterministic tools and quantum backends. DeepSeek Harness remains the runtime and isolation boundary.",
      },
    };
    ...
    function CapabilitySettingsSection({ api, loopback, t }) {
      ...
      const mcpCards = snapshot.mcpServers.map((server) => {
        ...
        return h("article", { className: "oq-cap-card", key: server.serverName },
          h("div", { className: "oq-cap-card-head" },
            h("div", { className: "oq-cap-title" }, h("h3", null, server.displayName), h("code", { className: "oq-cap-code" }, `${server.serverName} · ${server.provider}`)),
            h("span", { className: "oq-cap-badge", "data-tone": tone }, label),
          ),
          h("p", { className: "oq-cap-desc" }, server.description),
          h("p", { className: "oq-cap-meta" }, `${server.transport} · ${server.target}${server.packageVersion ? ` · ${server.packageVersion}` : ""}`),
          h("div", { className: "oq-cap-actions" },
      ...
      const skillCards = snapshot.skills.map((skill) => h("article", { className: "oq-cap-card", key: skill.name },
        h("div", { className: "oq-cap-card-head" },
          h("div", { className: "oq-cap-title" }, h("h3", null, skill.displayName), h("code", { className: "oq-cap-code" }, `/${skill.name}`)),
        ...
        h("p", { className: "oq-cap-meta" }, [skill.version, skill.maturity].filter(Boolean).join(" · ") || "项目 Skill"),
        h("div", { className: "oq-cap-actions" },
      ...
      return h("div", { className: "oq-cap-root" },
        h("section", { className: "oq-cap-hero" },
          h("p", { className: "oq-cap-eyebrow" }, "HARNESS CAPABILITIES"),
          h("h2", null, t("title")),
          h("p", null, t("summary")),
        ...
        h("div", { className: "oq-cap-tabs", role: "tablist", "aria-label": "能力分类" },
          [["mcp", "MCP 服务"], ["skills", "Skills"], ["credentials", "安全凭据"]].map(([id, label]) => h("button", { key: id, type: "button", role: "tab", className: "oq-cap-tab", "aria-selected": tab === id, onClick: () => setTab(id) }, label)),
        ),
