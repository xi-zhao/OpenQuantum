...
<p align="center">
  ...
  <a href="#rsi">长期愿景 · RSI</a> ·
  <a href="./docs/README.md">文档</a>
</p>

**让人、AI 与开放生态共同创造量子能力。**

OpenQuantum 汇聚专业量子软件、研究方法与完整应用，通过 AI Agent 连接问题与实际计算。你可以调用已有工具开展任务，直接使用已集成应用，也可以把自己的方法、算法、服务与产品接进来。

我们希望，个人能够组织超出自己熟悉工具范围的探索，研究成果能够成为下一项工作的起点，懂专业问题的人也能创造供他人使用的应用。学习、研究、开发与应用是不同入口，不是平台的边界。

今天，从[本地计算与量子云](#本地计算与量子云)、[量子学习通](#量子学习通)和[能力扩展](#把你的量子能力接进来)开始；长期探索量子计算、高性能计算（HPC）与 AI 的协作，以及让研究改善研究自身的 [RSI 愿景](#rsi)。当前接入能力、准备条件与验证范围逐项列在下文。
...
**当前状态：平台已有方法、执行与部分科学检查基础，尚未实现上述 RSI 闭环。** 改进提议、独立评估、版本接纳和跨轮收益仍需逐项建立与验证；[架构与证据边界](docs/README.md)继续作为当前实现依据。

我们最终希望看到的是：**一次研究产生新结果，也留下帮助下一次研究成功的能力。** 当前已交付能力及验证范围见[核心能力目录](#openquantum-的核心能力)。

## 一起建设 OpenQuantum

你不必从头开发一个平台，才能帮助量子能力向前一步。欢迎贡献可复现案例、领域 Skill、计算工具、完整应用、模型与运行环境适配、硬件接口和独立科学检查，也欢迎参与量子学习通的课程与实验设计。应用贡献可以从现有完整产品的适配开始，不必重新实现底层算法。

一个能被别人接续的研究案例、一份经过检查的负结果、一项减少重复配置的修复，或一个让学习者真正动手的实验，都有价值。请注明来源、作者、许可证、适用条件和验证范围；仅分享已获授权的材料；私人会话与未发表数据不默认公开，凭据和密钥不得进入贡献内容。
