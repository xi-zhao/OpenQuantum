...
<p align="center">
  ...
  <a href="#从一个量子任务开始">任务示例</a> ·
  <a href="#快速开始">快速开始</a> ·
  <a href="#把你的量子能力接进来">应用与扩展</a> ·
  <a href="#量子学习通">量子学习通</a> ·
  <a href="#为什么选择-openquantum">产品特点</a> ·
  <a href="./docs/README.md">文档</a>
</p>

OpenQuantum 将量子工具、专业方法与应用汇聚到一个开放平台，通过 AI Agent 连接用户需求与专业计算。你可以调用已有工具开展任务，直接使用已集成应用，也可以接入自己的算法、服务与产品。

**我们的目标是让任何量子相关需求，都能在这里找到合适的工具、应用或解决路径。** 平台不限定于某一类使用者或单一场景，而是持续扩展可使用的量子生态。

**生态汇聚**：专业工具、研究方法与完整应用，在一个平台中发现、使用和扩展。

**智能体驱动**：从提出需求到调用工具、推进计算，让专业量子软件进入实际任务流程。

**面向融合计算**：模型服务与计算后端分别配置，持续拓展量子计算、高性能计算与 AI 的协作方式。

当前已接入能力、依赖准备、使用条件与验证范围见下文；新工具与应用逐项完成适配。
...
OpenQuantum 不只把量子软件放在一起，而是把工具、方法与应用接入可使用的平台：直接使用完整应用，或让 Agent 协助选择方法、调用工具、推进计算。开源的适配实现与扩展接口，让已有能力能够持续复用和共同建设。

**生态汇聚：从软件目录到可使用的能力。** 专业工具通过执行接口接入，领域方法以 Skill 复用，完整应用保留自己的界面与业务流程。科研工作台和量子学习通是现有入口，更多量子应用逐项适配。平台不要求所有使用者都编写代码，也不要求所有应用都采用对话交互。

**智能体驱动：从对话问答到专业任务执行。** 领域 Skill 指导方法选择与步骤，Agent 调用专业工具执行计算，工作台保留请求、工具输入和返回结果。你可以继续调整参数、比较结果；独立参考与科学验收按相应能力的支持范围提供，不因接入平台而自动获得。

**面向量超智融合：让应用与多种计算资源衔接。** 模型服务与计算后端独立配置，工具通过各自接口接入，在已支持范围内按任务与资源条件选择组合。平台将持续拓展量子计算、高性能计算（HPC）与 AI 的协作，让应用不局限于单一模型或计算后端。当前接入范围见[本地计算与量子云](#本地计算与量子云)，远程 GPU、HPC 与更多 QPU 的协同在[长期发展规划](#长期发展规划)中逐项推进。

生态项目保留各自的来源、许可证与适用范围。你可以使用已接入的能力，也可以贡献工具适配、专业方法和完整应用，让这个开源量子 Agent 与应用平台随生态共同成长。
...
按需求选择下面已接入的能力。开展计算时，把问题、输入和希望检查的结果写进科研工作台的对话；OpenQuantum 的 Skill 提供工作方法，Agent 调用相应 Tool 完成计算。具体模型和参数见[本地计算说明](docs/integrations/SCALABLE_BRIDGES.md)；学习与教学可进入[量子学习通](#量子学习通)，完整应用接入见[应用与扩展](#接入面向实际问题的量子应用)。
...
在科研工作台新建对话，复制前面的 [Bell 态任务](#从一个量子任务开始)，查看工具返回的精确概率与采样频数，再继续调整采样量。FatQat 连接默认开启，首次使用可能下载 Python 依赖。

需要其他后端时，在「设置 → 量子组件 → MCP Server 连接」中启用相应连接并重启工作台，再在对话中指定名称。例如，启用 TyxonQ 后，可以请求用它制备同一个 Bell 态并返回精确态矢。对话模型和计算后端分别选择；默认开关与准备条件见[服务目录](#mcp-服务目录)。

<details>
<summary><strong>更多可复制的计算任务与依赖准备</strong></summary>
...
五项计算通过 uv 准备各自的 Python 3.12 环境，无需量子云账户；Dynamiqs 使用已安装的 JAX 后端。升级后重启工作台，在科研对话中复制一条请求：
...
<details>
<summary><strong>开始前的几个问题</strong></summary>
...
其他部署方式、模型配置和故障定位见[部署与启动](docs/DEPLOYMENT.md)与[故障排查](docs/TROUBLESHOOTING.md)。
...
本地计算无需量子云账户。需要真实硬件时，可以按任务选择 IBM Quantum、IonQ、本源量子等接口；其他国内量子云可通过 FieldQKit 发现设备和筛选拓扑。云任务需配置相应凭据、权限与额度，再启用对应连接。
...
量子学习通是平台已集成的应用之一，面向学习与教学需求。学习者可以从课程材料与课堂活动开始，教育者可以准备和组织教学内容。

### 量子学习通

量子学习通是 OpenQuantum 的学习与教学应用，把材料、课件和课堂放在一起，支持自主学习与授课。应用名称与主题适配 OpenQuantum，保留课程内容自身的样式；其教学应用基础与适配关系见后文[开源生态与致谢](#开源生态与致谢)。
...
<details>
<summary><strong>桌面客户端安装</strong></summary>

### 桌面客户端

桌面端的界面与平台状态见[桌面与消息入口](#在桌面和消息中使用)。
...
<details>
<summary><strong>微信、飞书与其他消息入口配置</strong></summary>
...
<details>
<summary><strong>开发架构与验证命令</strong></summary>
...
<details>
<summary><strong>内置 Skills：用途与执行入口</strong></summary>
...
<details>
<summary><strong>MCP 服务目录：连接、默认开关与准备条件</strong></summary>

### MCP 服务目录

**OpenQuantum 为 22 项计算与设备发现能力开发了本地 MCP 桥接**，把所用 SDK 或数值库转换成 Agent 可调用的 Tool。下表先列出这些桥接实现：连接名链接到本仓库源码，旁边保留所用上游的链接；末尾再列 7 个直接接入的上游 MCP 服务。
...
**以满足各种量子相关需求为长期目标。** 围绕开源量子 Agent 与应用平台，持续推进生态汇聚、智能体执行与量超智融合，让专业方法、计算资源、完整应用、实验仪器和学习资源各有入口，并在适用范围内协作。平台不限定于某一类人群或单一场景，具体能力通过逐项接入与验证交付。
...
以上是长期建设方向，当前已交付能力及验证范围见[核心能力目录](#openquantum-的核心能力)。欢迎带着研究方法、完整应用、算力资源、仪器接口或教学经验，[一起建设 OpenQuantum](#一起建设-openquantum)。

## 一起建设 OpenQuantum

欢迎贡献可复现案例、领域 Skill、计算工具、完整量子应用、模型与运行环境适配、硬件接入和独立科学检查，也欢迎参与量子学习通的课程设计与资源整理，注明来源、适用基础和授权范围。应用贡献可以从现有完整产品的适配开始，不必重新实现底层算法。新增内容应服务明确的学习、教学、研究或应用任务，并说明当前实现与验证范围。

从[贡献指南](CONTRIBUTING.md)开始，产品背景见[项目故事](docs/communications/openquantum-wechat-launch.md)，安全问题请按[安全政策](SECURITY.md)私密报告。

先按需求[浏览已有能力](#可以用它做什么)，或直接[开始使用](#快速开始)。开展计算可以[复算一个本地示例](#从一个量子任务开始)，学习与教学可进入[量子学习通](#量子学习通)。已有自己的量子方法、工具或完整应用，则从[扩展入口](#把你的量子能力接进来)开始。
...
<details>
<summary><strong>可选上游 Skill 与开发证据</strong></summary>
